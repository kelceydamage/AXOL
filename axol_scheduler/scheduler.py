...
class Task(object):
    def __init__(self, name, start_time, calc_next_time, time_interval, func, axol_node, api_name, method, role):
        """
        Initialize a Task.
        ...
        if not self.halt_flag.isSet():
            try:
                try:
                    ...
                    print 'Successfully ran scheduled task %s: Interval %s' % (
                        self.name,
                        self.time_interval
                        )
                except:
                ...
                print 'Scheduled Time:', self.scheduled_time
                self.scheduled_time = self.calc_next_time(
                    self.scheduled_time
                    )

                print 'Next Time:', self.scheduled_time
                logging.debug(
                    "Scheduled next run of %s for: %s" % (
                        self.name,
                        self.scheduled_time,
                        )
                    )
...
class Scheduler(Thread):
    ...
    def schedule(self, name, start_time, calc_next_time, time_interval, func, axol_node, api_name, method, role):
        task = Task(
            name,
            start_time,
            calc_next_time,
        ...
        receipt = self.schedule_task(
            task
            )
        print 'New Task: %s has been scheduled with an interval of %s' % (
            name,
            time_interval
            )
        return receipt
    ...
    def run(self):
        while 1:
            ...
            if receipt != None:
                ...
                logging.debug(
                    "Next task is %s in %s seconds" % (
                        self.tasks[receipt].name,
                        time_to_wait,
                        )
                    )
                self.halt_flag.wait(
...
def daily_at(time):
    ...
    """
    return lambda last: datetime.datetime.combine(
