...
DEFAULT_CELERYD="--detach" #"-m celery.bin.celeryd_detach"
...
#CELERYD_OPTS="$CELERYD_OPTS -B -E -v 2"
...
start_workers () {
    $CELERYD_MULTI start worker --app="axol_core" $DAEMON_OPTS \
...
restart_workers () {
    $CELERYD_MULTI restart worker --app="axol_core" $DAEMON_OPTS \
                           --pidfile="$CELERYD_PID_FILE" \
                           --logfile="$CELERYD_LOG_FILE" \
                           --loglevel="$CELERYD_LOG_LEVEL" \
                           $CELERYD \
